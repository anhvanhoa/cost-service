...
CREATE TABLE cost_trackings (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(), -- Mã định danh duy nhất cho mỗi chi phí (UUID)
    
    planting_cycle_id UUID, -- Tham chiếu đến chu kỳ trồng trọt liên quan đến khoản chi phí
    ...
    receipt_image UUID, -- Tham chiếu đến bảng media lưu hình ảnh biên lai
    
    created_by UUID, -- Người tạo bản ghi (tham chiếu đến bảng users)
...
CREATE INDEX idx_cost_tracking_cycle_category ON cost_trackings (planting_cycle_id, cost_category); -- Index tìm kiếm theo chu kỳ và loại chi phí
CREATE INDEX idx_cost_tracking_date ON cost_trackings (purchase_date); -- Index tìm kiếm theo ngày phát sinh
CREATE INDEX idx_cost_tracking_supplier ON cost_trackings (supplier); -- Index tìm kiếm nhanh theo nhà cung cấp

-- Create trigger for updated_at column
CREATE TRIGGER trigger_cost_tracking_updated_at
    BEFORE UPDATE ON cost_trackings
    FOR EACH ROW
